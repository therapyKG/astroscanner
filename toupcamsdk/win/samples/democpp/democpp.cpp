			...
			if (SUCCEEDED(Toupcam_get_PixelFormatSupport(m_hcam, -1, &num)) && (num > 0))
			{
				for (int i = 0; i < num; ++i)
				{
					if (SUCCEEDED(Toupcam_get_PixelFormatSupport(m_hcam, (char)i, &val)))
						cbox.SetItemData(cbox.AddString(CA2W(Toupcam_get_PixelFormatName(val))), val);
				}
			}
		}
...
private:
	...
	void OnEventImage()
	{
		...
		HRESULT hr = Toupcam_PullImageV3(m_hcam, m_pData, 0, m_header.biBitCount, 0, &info);
		if (FAILED(hr))
			return;
	...
	void OnEventSnap()
	{
		...
		ToupcamFrameInfoV3 info = { 0 };
		HRESULT hr = Toupcam_PullImageV3(m_hcam, NULL, 1, 24, 0, &info); //first, peek the width and height
		if (SUCCEEDED(hr))
		{
			...
			if (pSnapData)
			{
				hr = Toupcam_PullImageV3(m_hcam, pSnapData, 1, 24, 0, NULL);
				if (SUCCEEDED(hr))
				{
					if (2 == m_nSnapType)
					{
	...
	void UpdateFrameText(const ToupcamFrameInfoV3& info)
	{
		wchar_t str[256];

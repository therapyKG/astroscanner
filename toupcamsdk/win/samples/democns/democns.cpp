...
static bool IsSupportGain(const ToupcamModelV2* m) noexcept
{
	return (m->flag & TOUPCAM_FLAG_GHOPTO) ? false : true;
}
...
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		else
		{
			...
			for (int i = 0; i < num; ++i)
			{
				if (SUCCEEDED(Toupcam_get_PixelFormatSupport(m_hcam, (char)i, &val)))
				{
					box.SetItemData(box.AddString(CA2W(Toupcam_get_PixelFormatName(val))), val);
					if (dwValue == val)
		...
		if (m_pModel->flag & TOUPCAM_FLAG_TEC_ONOFF)
		{
			...
			val = TOUPCAM_TEC_TARGET_DEF;
			m_regkey.QueryDWORDValue(L"tectarget", val);
			int tectarget = (int)val;
			if (tectarget < TOUPCAM_TEC_TARGET_MIN)
				tectarget = TOUPCAM_TEC_TARGET_MIN;
			else if (tectarget > TOUPCAM_TEC_TARGET_MAX)
				tectarget = TOUPCAM_TEC_TARGET_MAX;
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (m_pModel->flag & TOUPCAM_FLAG_TEC_ONOFF)
		{
			...
			if (bEnable)
			{
				...
				if ((n < TOUPCAM_TEC_TARGET_MIN) || (n > TOUPCAM_TEC_TARGET_MAX))
				{
					AtlMessageBox(m_hWnd, (LPCTSTR)FormatString(L"Tec target out of range [%.1f, %.1f].", TOUPCAM_TEC_TARGET_MIN / 10.0, TOUPCAM_TEC_TARGET_MAX / 10.0), (LPCTSTR)nullptr, MB_OK | MB_ICONWARNING);
					return 0;
				}
	...
	{
		CExposureDlg dlg(IsSupportGain(m_pModel), m_expoTimeRange, m_expoGainRange);
		if (IDOK == dlg.DoModal())
		{
	...
	LRESULT OnNmDblclk(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/)
	{
		...
		{
			CExposureDlg dlg(IsSupportGain(m_pModel), m_expoTimeRange, m_expoGainRange, m_vecExpo[pNMITEMACTIVATE->iItem]);
			if (IDOK == dlg.DoModal())
			{
...
private:
	...
	template<typename T>
	void GetData(int* arr, const T* pData, const ToupcamFrameInfoV3& info)
	{
	...
	void OnEventImage()
	{
		...
		const HRESULT hr = Toupcam_PullImageV3(m_hcam, m_pRawData, 0, 0, 0, &info);
		if (SUCCEEDED(hr))
		{
			if (m_bTriggerMode)
				bAdd = true;

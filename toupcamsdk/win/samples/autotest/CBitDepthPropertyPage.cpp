...
BOOL CBitDepthPropertyPage::OnInitDialog()
{
	...
	if (g_hcam)
	{
		const int maxBit = Toupcam_get_MaxBitDepth(g_hcam);
		if (maxBit <= 8)
		...
		Toupcam_get_Option(g_hcam, TOUPCAM_OPTION_BITDEPTH, &m_bitDepth);
		UpdateData(FALSE);
	}
...
{
	Toupcam_put_Option(g_hcam, TOUPCAM_OPTION_BITDEPTH, m_bitDepth);
}

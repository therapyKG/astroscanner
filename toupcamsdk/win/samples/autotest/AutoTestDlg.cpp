...
void CAutoTestDlg::OnEventImage()
{
	...
	HRESULT hr = Toupcam_PullImageV3(g_hcam, m_pImageData, 0, 24, 0, &info);
	if (SUCCEEDED(hr))
	{
...
{
	ToupcamFrameInfoV3 info = { 0 };
	HRESULT hr = Toupcam_PullImageV3(g_hcam, nullptr, 1, 24, 0, &info);
	if (SUCCEEDED(hr))
	{
		void* pData = malloc(TDIBWIDTHBYTES(info.width * 24) * info.height);
		hr = Toupcam_PullImageV3(g_hcam, pData, 1, 24, 0, nullptr);
		if (SUCCEEDED(hr))
		{
			BITMAPINFOHEADER header = { 0 };
			header.biSize = sizeof(header);

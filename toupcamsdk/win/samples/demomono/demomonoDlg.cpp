...
void CdemomonoDlg::OnEventImage()
{
	...
	HRESULT hr = Toupcam_PullImageV3(m_hcam, m_pImageData, 0, m_bBitDepth ? 16 : 8, 0, &info);
	if (SUCCEEDED(hr))
	{
...
{
	ToupcamFrameInfoV3 info = { 0 };
	HRESULT hr = Toupcam_PullImageV3(m_hcam, NULL, 1, m_bBitDepth ? 16 : 8, 0, &info);
	if (SUCCEEDED(hr))
	{
		void* pData = malloc(TDIBWIDTHBYTES(info.width * 16) * info.height);
		hr = Toupcam_PullImageV3(m_hcam, pData, 1, m_bBitDepth ? 16 : 8, 0, NULL);
		if (SUCCEEDED(hr))
		{

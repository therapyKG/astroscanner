...
{
	bool bLoadStatus;
	Toupcam_read_EEPROM(m_hcam, 0, (unsigned char*)&bLoadStatus, sizeof(bool));
	if (bLoadStatus)
	{
	...
	Toupcam_put_AFMode(m_hcam, ToupcamAFMode_CALIBRATE);
}
...
void CdemoafDlg::OnCbnSelchangeCombo1()
{
	...
	{
		Toupcam_put_AFMode(m_hcam, ToupcamAFMode_NONE);
		...
		StartDevice();
		Toupcam_put_AFMode(m_hcam, ToupcamAFMode_MANUAL);
	}
}
...
void CdemoafDlg::OnEventImage()
{
	...
	HRESULT hr = Toupcam_PullImageV3(m_hcam, m_pImageData, 0, 24, 0, &info);
...
{
	ToupcamFrameInfoV3 info = { 0 };
	HRESULT hr = Toupcam_PullImageV3(m_hcam, NULL, 1, 24, 0, &info);
	if (SUCCEEDED(hr))
	{
...
void CdemoafDlg::OnTimer(UINT_PTR nIDEvent)
{
	if (1 == nIDEvent)
	{
		...
		UpdateData(TRUE);
		ToupcamAFState afStatus;
		if (SUCCEEDED(Toupcam_get_AFState(m_hcam, &afStatus)) && SUCCEEDED(Toupcam_get_LensInfo(m_hcam, &m_afLensInfo)))
		{
...
void CdemoafDlg::SetLensStatus()
{
	...
	Toupcam_read_EEPROM(m_hcam, 0, (unsigned char*)&SetStatus, sizeof(LensStatusInEEPROM));
	if (SetStatus.usID == m_afLensInfo.lensID)
	{
		...
		Toupcam_put_AFAperture(m_hcam, SetStatus.cFNumber_Index);
		Toupcam_put_AFFMPos(m_hcam, SetStatus.sFM_Cur);

		if (0 < SetStatus.uiExpTime && SetStatus.uiExpTime < 1000)
		{
			Toupcam_put_ExpoTime(m_hcam, SetStatus.uiExpTime * 1000);
		...
		{
			Toupcam_put_ExpoAGain(m_hcam, SetStatus.usGain);
...
void CdemoafDlg::OnBnClickedStatusDefault()
{
	...
	Toupcam_write_EEPROM(m_hcam, 0, (const unsigned char*)&SetStatus, sizeof(LensStatusInEEPROM));
}
...
{
	Toupcam_put_AFMode(m_hcam, ToupcamAFMode_AUTO);
	SetFocusFNControll(FALSE);
	GetDlgItem(IDC_LENSCAL)->EnableWindow(TRUE);
	GetDlgItem(IDC_RADIO_MANUAL)->EnableWindow(TRUE);
	GetDlgItem(IDC_BUTTON_ONEPUSH)->EnableWindow(FALSE);
...
void CdemoafDlg::OnBnClickedButtonOnepush()
{
	...
	{
		Toupcam_put_AFMode(m_hcam, ToupcamAFMode_ONCE);
		((CButton*)GetDlgItem(IDC_RADIO_MANUAL))->SetCheck(FALSE);
		((CButton*)GetDlgItem(IDC_RADIO_AUTO))->SetCheck(FALSE);
		SetFocusFNControll(FALSE);
		GetDlgItem(IDC_LENSCAL)->EnableWindow(FALSE);
...
void CdemoafDlg::OnBnClickedButtonSaveStatus()
{
	...
	Toupcam_write_EEPROM(m_hcam, 0, (const unsigned char*)&SaveStatus, sizeof(LensStatusInEEPROM));
}

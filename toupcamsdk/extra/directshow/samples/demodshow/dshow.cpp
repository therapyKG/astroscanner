...
HRESULT TDshowContext::set_roi(unsigned xOffset, unsigned yOffset, unsigned xWidth, unsigned yHeight)
{
	...
	IToupcam* pIToupcam = NULL;
	if (m_pSource && SUCCEEDED(m_pSource->QueryInterface(IID_IToupcam, (void**)&pIToupcam)) && pIToupcam)
	{
		StopPreview();
		if (m_fPreviewGraphBuilt)
			TearDownGraph();
		hr = pIToupcam->put_Roi(xOffset, yOffset, xWidth, yHeight);
		pIToupcam->Release();
...
HRESULT TDshowContext::get_roi(unsigned* pxOffset, unsigned* pyOffset, unsigned* pxWidth, unsigned* pyHeight)
{
	...
	if (m_pSource && SUCCEEDED(m_pSource->QueryInterface(IID_IToupcam, (void**)&pIToupcam)) && pIToupcam)
	{
		hr = pIToupcam->get_Roi(pxOffset, pyOffset, pxWidth, pyHeight);
		pIToupcam->Release();
	}
	return hr;
}
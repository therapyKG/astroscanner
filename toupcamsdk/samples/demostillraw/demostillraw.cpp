...
static void __stdcall EventCallback(unsigned nEvent, void* pCallbackCtx)
{
    ...
    {
        ToupcamFrameInfoV3 info = { 0 };
        HRESULT hr = Toupcam_PullImageV3(g_hcam, nullptr, 1, 0, 0, &info); //peek width & height
        if (FAILED(hr))
        ...
        else
        {
            ...
            {
                /* After we get the image data, we can do anything for the data we want to do */
                hr = Toupcam_PullImageV3(g_hcam, pStillImage, 1, 0, 0, &info);
                if (FAILED(hr))
                    printf("failed to pull still image, hr = %08x\n", hr);
                else
                {
            ...
            else
            {
                ...
                do {
                    ...
                    if (fgets(str, 1023, stdin))
                    {
                        if (('x' == str[0]) || ('X' == str[0]))
                            break;
                        Toupcam_SnapR(g_hcam, atoi(str), 1);
                    }
                } while (true);
            }
        }

...
Friend Class Toupcam
    ...
    Public Enum eOPTION As UInteger
       ...
       OPTION_DFC = &H1D                           ' dark field correction
        ...
        OPTION_AFSTATUS = &H27                     ' auto focus status, see ToupcamAFStaus
        ...
        OPTION_FRONTEND_DEQUE_LENGTH = &H31        ' frontend (raw) frame buffer deque length, range: [2, 1024], default: 4
                                                   ...
                                                   '
        OPTION_FRAME_DEQUE_LENGTH = &H31           ' alias of TOUPCAM_OPTION_FRONTEND_DEQUE_LENGTH
        ...
        OPTION_SEQUENCER_EXPOTIME = &H1000000      ' sequencer trigger: exposure time, iOption = TOUPCAM_OPTION_SEQUENCER_EXPOTIME | index, iValue = exposure time
                                                   ...
                                                   '
        OPTION_SEQUENCER_EXPOGAIN = &H2000000      ' sequencer trigger: exposure gain, iOption = TOUPCAM_OPTION_SEQUENCER_EXPOGAIN | index, iValue = gain
        ...
        OPTION_CHAMBER_HT = &H4C                   ' get chamber humidity & temperature:
                                                   ...
                                                   '
        TOUPCAM_OPTION_ENV_HT = &H4D               ' get environment humidity & temperature
        ...
        OPTION_FPNC = &H67                         ' Fix Pattern Noise Correction
                                                   '        set:
                                                   '             0: disable
                                                   '            1: enable
                                                   '           -1: reset
...
#End If
    ...
    ' get the version of this dll, which is: 55.24446.20240114
...
#End If
    ...
    Public Function TriggerSync(nTimeout As UInteger, pImageData As IntPtr, bits As Integer, rowPitch As Integer, ByRef pInfo As FrameInfoV3) As Boolean
        ...
        Return CheckHResult(Toupcam_TriggerSync(handle_, nTimeout, pImageData, bits, rowPitch, pInfo))
    End Function
    ...
    Public Function get_PixelFormatSupport(cmd As SByte, ByRef iValue As Integer) As Boolean
        If handle_ Is Nothing OrElse handle_.IsInvalid OrElse handle_.IsClosed Then
            ...
            Return False
        End If
        Return CheckHResult(Toupcam_get_PixelFormatSupport(handle_, cmd, iValue))
    End Function
    ...
    Public Shared Function PixelFormatName(val As Integer) As String
        Dim ptr As IntPtr = Toupcam_get_PixelFormatName(val)
...
#End If
    ...
    End Function
    <DllImport("toupcam.dll", ExactSpelling:=True, CallingConvention:=CallingConvention.Winapi)>
    Private Shared Function Toupcam_TriggerSync(h As SafeCamHandle, nTimeout As UInteger, pImageData As IntPtr, bits As Integer, rowPitch As Integer, ByRef pInfo As FrameInfoV3) As Integer
    End Function
    <DllImport("toupcam.dll", ExactSpelling:=True, CallingConvention:=CallingConvention.Winapi)>
    Private Shared Function Toupcam_TriggerSync(h As SafeCamHandle, nTimeout As UInteger, pImageData As Byte(), bits As Integer, rowPitch As Integer, ByRef pInfo As FrameInfoV3) As Integer
    End Function
    <DllImport("toupcam.dll", ExactSpelling:=True, CallingConvention:=CallingConvention.Winapi)>
    Private Shared Function Toupcam_TriggerSync(h As SafeCamHandle, nTimeout As UInteger, pImageData As UShort(), bits As Integer, rowPitch As Integer, ByRef pInfo As FrameInfoV3) As Integer
    End Function
